...
| 4 | Same → `submission_text_baseline.csv` | 0.9707 | | _pending_ | Needs `~/.kaggle/kaggle.json` + Notebook submit; test has no Report | 2026-08-07 |
...
```bash
.venv/bin/python train_text_baseline.py --cv 5
.venv/bin/python train_text_improved.py --cv 5
.venv/bin/python train_image_baseline.py --cv 5
.venv/bin/python fuse_predictions.py
...
- Synthetic local AUCs are optimistic; replace data via `bash scripts/download_csvs.sh` once credentials exist.
